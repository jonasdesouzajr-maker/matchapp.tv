...
const absolutePoster=e=>poster(e).startsWith('http')?poster(e):SITE+poster(e).split('?')[0];
...
<a class="event-poster-link" href="/events/${esc(e.slug)}/"><img src="${esc(poster(e))}" alt="${esc(e.title)} — official event artwork" width="1024" height="1536" loading="lazy" decoding="async"></a>
...
<header class="app-header"><a href="/" class="header-brand-area"><img class="brand-logo" src="/logo.jpeg?v=2" width="60" height="60" alt="MatchApp"><span class="app-title-main">MatchApp</span></a><nav class="event-top-nav" aria-label="Event navigation"><a class="event-nav-pill event-nav-home" href="/"><span class="event-nav-ico" aria-hidden="true">⌂</span><span>Home</span></a><a class="event-nav-pill event-nav-events" href="/events-archive.html" aria-current="page"><span class="event-nav-ico" aria-hidden="true">✦</span><span>Events</span></a><a class="event-nav-pill event-nav-kids" href="/kids/"><img src="/kids/kids-logo-sm.jpeg?v=2" width="22" height="22" alt="" aria-hidden="true"><span>Kids Mode</span></a></nav></header>
...
for(const e of events){
 ...
 const body=`<article class="event-guide" ${data(e)}><div class="event-detail-hero"><div class="event-hero-art"><img src="${esc(poster(e))}" alt="${esc(e.title)} — official event artwork" width="1024" height="1536" fetchpriority="high"></div><div class="event-hero-copy"><div class="event-guide-flags">${country(e)}<span class="event-category">${esc(e.category)}</span></div><span data-event-status class="event-badge event-soon">Upcoming</span>${countdown()}<h1>${esc(e.title)}</h1><p class="event-date-line">${date(e)}<br>${esc(e.location)}</p><p class="event-synopsis">${esc(e.synopsis)}</p><p class="event-art-credit">${esc(e.posterCredit||'Official event artwork')}</p><div class="event-primary-actions"><a class="gold-btn" href="${esc(askEvent(e))}">Open in MatchApp Ai</a><a class="event-secondary-btn" href="${esc(e.official)}" target="_blank" rel="noopener noreferrer">Official event</a></div></div></div>
...
const archiveBody=`<section class="event-archive-hero"><p class="event-eyebrow">MATCHAPP VERIFIED EVENTS</p><h1>Upcoming global entertainment events</h1><p>Film festivals, pop-culture conventions, gaming, sports and esports across the United States, United Kingdom, Canada, Brazil, Australia, Japan and beyond — with date-aware status, official sources and MatchApp Ai handoff.</p></section><div class="global-event-grid event-archive-grid">${cards}</div>`;
fs.writeFileSync(path.join(ROOT,'events-archive.html'),shell({headHtml:head({title:'Upcoming entertainment events — US, UK, Canada, Brazil, Australia & Japan',description:'Verified MatchApp guides to current and upcoming entertainment events in the United States, United Kingdom, Canada, Brazil, Australia, Japan and beyond, with dates, official programmes and title discovery.',url:SITE+'/events-archive.html',image:absolutePoster(events.find(e=>e.slug==='new-york-film-festival-2026')||events[0]),keywords:events.map(e=>e.keywords).join(', '),schema:archiveSchema,ads:true}),body:archiveBody,archive:true}));

const homeSection=`<section id="global-events" class="events-wrapper"><details class="global-events-fold" open><summary class="global-events-summary"><span class="ge-sum-label"><span data-i18n="global.heading">Upcoming global events</span></span><span class="ge-sum-count">${events.length}</span><span class="ge-sum-chev" aria-hidden="true">⌄</span></summary><div class="global-events-body"><p class="ge-help">Tap a poster for a MatchApp Ai event guide, official programme links and title discovery.</p><div class="events-rail-shell"><button type="button" class="marquee-arrow events-prev" data-rail-dir="-1" aria-label="Scroll global events left" aria-controls="events-viewport">‹</button><button type="button" class="marquee-arrow events-next" data-rail-dir="1" aria-label="Scroll global events right" aria-controls="events-viewport">›</button><div class="global-event-grid events-viewport" id="events-viewport" role="region" aria-label="Upcoming global event posters" tabindex="0">${cards}</div></div><a class="ge-archive gold-btn" href="/events-archive.html">All event guides</a></div></details></section>`;
