...
- The adult module is `:app`: version 1.1.31 (code 33), compile/target API 36. `:kidsapp` remains separate and unchanged.
...
5. Generate a signed **Android App Bundle** for `:app` only, using a private signing key not stored in Git. Verify package `com.jonas.papercup`, version code 33, target SDK 36, correct upload signing certificate and Play Console listing/data safety declarations.
