 ...
 *
 * GitHub Actions obtains a short-lived OIDC token and this function verifies
 * issuer, audience, repository, workflow and event claims before any write.
 * TMDB and Supabase admin credentials remain Supabase Function secrets.
 * Streaming availability comes from TMDB's official watch/providers data
 * (provider data supplied by JustWatch), not brittle third-party HTML scraping.
 */
...
async function tmdb(path:string,token:string):Promise<any|null>{
  ...
  try{const res=await fetch(`${TMDB_BASE}${path}`,{signal:ac.signal,headers:{authorization:`Bearer ${token}`,accept:"application/json"}});if(!res.ok){console.error(JSON.stringify({event:"catalog-sync-tmdb-error",path:path.split("?")[0],status:res.status}));return null;}return await res.json();}
  catch(error){console.error(JSON.stringify({event:"catalog-sync-tmdb-fetch-failed",path:path.split("?")[0],error:String(error)}));return null;}finally{clearTimeout(timer);}
}
...
function availabilityFrom(record:any){const source=record?.["watch/providers"]?.results||{},out:Record<string,unknown>={source:"tmdb-watch-providers",attribution:"JustWatch via TMDB"};for(const region of REGIONS){const r=source?.[region];if(!r)continue;out[region]={link:typeof r.link==="string"&&/^https:\/\//.test(r.link)?r.link:null,stream:[...providerNames(r.flatrate),...providerNames(r.free),...providerNames(r.ads)].filter((v,i,a)=>a.indexOf(v)===i),rent:providerNames(r.rent),buy:providerNames(r.buy)};}return out;}
...
async function resolveTmdb(item:any,token:string){
  const kind:"movie"|"tv"=item.media_kind==="movie"?"movie":"tv",q=encodeURIComponent(String(item.title||"").trim());if(!q)return null;
  ...
  const exact=rows.find((r:any)=>{if(r?.adult===true||!Number.isSafeInteger(r?.id))return false;const ok=[titleOf(r,kind),originalTitleOf(r,kind)].some(t=>normalise(t)===target);if(!ok)return false;if(!item.year)return true;const y=yearOf(r,kind);return y!==null&&Math.abs(y-Number(item.year))<=1;});if(!exact)return null;
...
Deno.serve(async(req)=>{
  ...
  try{const claims=await authorize(req),tmdbToken=Deno.env.get("TMDB_API_KEY");if(!tmdbToken)return json({error:"TMDB_API_KEY is not configured"},503);const body=await req.json().catch(()=>({})),incoming=Array.isArray(body?.catalog)?body.catalog:[],clean=incoming.filter((x:any)=>x&&typeof x.title==="string"&&x.title.trim().length>0&&x.title.length<=240).map((x:any)=>({title:x.title.trim(),year:Number.isInteger(Number(x.year))?Number(x.year):null,media_kind:["movie","tv","music","podcast","audiobook","audio","other"].includes(x.media_kind)?x.media_kind:"other",kids_approved:x.kids_approved===true,kids_age_bands:Array.isArray(x.kids_age_bands)?x.kids_age_bands.map(String).slice(0,8):[]}));
    ...
    const summary={ok:true,requested:clean.length,catalog_attempted:selected.length,catalog_resolved:catalogRows.length,trending_resolved:trendingRows.length,upserted:rows.length,duration_ms:Date.now()-started,ref:String(claims.ref||"")};console.log(JSON.stringify({event:"catalog-sync-summary",...summary}));return json(summary);
  }catch(error){console.error(JSON.stringify({event:"catalog-sync-failed",duration_ms:Date.now()-started,error:String(error)}));return json({error:"catalog sync rejected or failed"},403);}
});
