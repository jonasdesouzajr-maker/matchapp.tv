...
export async function verifiedCheckout(stripe:any,session:any):Promise<{user:string;product:CatalogEntry}|null>{
 ...
 const price=items.data[0].price;
 // Adaptive Pricing may present the session in a local currency; the verified
 // merchant Price identity and its base currency remain authoritative.
 const product=allowed.find(p=>p.price===price?.id&&p.mode===session.mode&&p.currency===price.currency);
 if(!product)return null;
 // Legacy merchant Payment Links or sessions generated by our authenticated checkout.
