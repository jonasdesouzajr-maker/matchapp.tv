...
begin
 if p_reason='match' then return public.consume_match(); end if;
 if p_reason='ask_ai' then
  v_result:=public.consume_credit('ask_ai');
  if coalesce((v_result->>'ok')::boolean,false) then
   return jsonb_build_object('allowed',true,'credits',coalesce((v_result->>'credits')::integer,0),'ask_ai',true);
  end if;
  return jsonb_build_object('allowed',false,'reason',case when v_result->>'reason'='no_credits' then 'no_credits' else v_result->>'reason' end,'credits',coalesce((v_result->>'credits')::integer,0),'ask_ai',true);
 end if;
