...
- **Sports source contingency:** the GDELT DOC API timed out on GitHub-hosted runners over both default TLS and forced IPv4 on 25 September 2026. When `NEWSDATA_API_KEY` is configured as an Actions repository secret, the existing sports discovery script first queries NewsData.io's commercially permitted Latest News sports category (free plan typically ~12 hours delayed), then independently checks direct HTTPS URLs against the existing trusted publisher domain list. Never store a key in tracked files, logs or client-side code. If this source fails or is not configured, the existing public GDELT attempt remains; if all sources fail, do not overwrite prior committed news or invent article links. Configure the repository secret and trigger the existing sports workflow manually to validate live delivery. See `docs/SPORTS_NEWS_SOURCE_SETUP.md`.
