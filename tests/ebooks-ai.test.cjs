...
test('grown-up Home loads E-books Ai data, current suggestions and matcher in safe order',()=>{
 ...
 const cover=html.indexOf('/ebooks/cover-identity.js?v=20260925-exact1');
 const matcher=html.indexOf('/ebooks/ebook-matcher.js?v=20260925-exact1');
 assert(catalog>0);
 assert(top>catalog);
 assert(audio>top);
 assert(cover>audio);
 ...
 assert.match(html,/\/ebooks\/ebook-matcher\.css\?v=20260925-home1/);
});
