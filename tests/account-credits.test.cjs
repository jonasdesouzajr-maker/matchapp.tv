...
test('restored signed-in sessions use one shared server balance for Ask AI and matches, without a local paid fallback',async()=>{
 const d=new JSDOM('',{url:'https://matchapp.tv/discover.html',runScripts:'outside-only'}),w=d.window,calls=[];let release,credits=2;
 w.supabaseClient={auth:{getSession:()=>new Promise(r=>release=()=>r({data:{session:{user:{id:uid}}}}))},rpc:async(name,args)=>{calls.push([name,args.p_reason]);return {data:{allowed:true,remaining:0,credits:--credits,paid_with_credit:true}};}};
 ...
 const match=w.checkDailyLimit();await tick();release();assert.equal(await match,true);assert.deepEqual(calls,[['consume_ai_action','ask_ai'],['consume_ai_action','match']]);assert.equal(credits,0);
 w.supabaseClient.auth.getSession=async()=>({data:{session:{user:{id:uid}}}});w.supabaseClient.rpc=async()=>({error:{message:'offline'}});assert.equal(await w.checkDailyLimit('ask_ai'),false);assert.equal(credits,0);
...
test('database credits serve both actions, free allowance comes first, identity is immutable and client-created paid balances are forbidden',async()=>{
 ...
 const consume=reason=>db.query('select public.consume_ai_action($1) r',[reason]);let r=(await consume('ask_ai')).rows[0].r;assert.equal(r.credits,2);assert.equal(r.remaining,1);await consume('match');assert.equal((await consume('ask_ai')).rows[0].r.credits,1);assert.equal((await db.query('select public.consume_match() r')).rows[0].r.credits,0);assert.equal((await consume('ask_ai')).rows[0].r.allowed,false);
 await db.exec('reset role');assert.deepEqual((await db.query('select reason from public.credit_ledger')).rows.map(r=>r.reason),['ask_ai','match']);
