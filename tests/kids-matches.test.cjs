...
async function boot(auth,quota={allowed:true,remaining:2},url='https://matchapp.tv/kids/'){
 ...
 w.supabaseClient={auth:{getSession:auth||asyncSession},rpc:async(name,args)=>{calls.push({name,args});return {data:name==='consume_ai_action'?quota:{history:[],keys:[]}};}};
...
test('Kids poster opens a metered full match; double taps, viewing, sharing and history do not charge twice',async()=>{
 ...
 const consumes=()=>calls.filter(c=>c.name==='consume_ai_action');assert.equal(consumes().length,1);assert.equal(consumes()[0].args.p_reason,'match');assert(w.document.getElementById('kids-watch-dialog').open);assert.equal(w.document.getElementById('kids-watch-name').textContent,'Bluey');assert.match(w.document.getElementById('kids-match-detail').textContent,/A história/);
 ...
 w.document.querySelector('[data-match-choice=save]').click();await tick();assert.equal(calls.filter(c=>c.name==='portfolio_action'&&c.args.p_action==='remember').length,1);assert.equal(w.matchPolicy.history()[0].action,'save');assert(w.matchPolicy.known().has(w.matchPolicy.key('Bluey')));assert.equal(consumes().length,1);d.window.close();
});
test('restoring a session never spends guest quota, and an age change before restoration cancels the match',async()=>{
 let release;const {w,d,calls}=await boot(()=>new Promise(r=>release=r));w.document.querySelector('#kids-grid [data-title="Bluey"] .kids-card-poster').click();await tick();assert.equal(calls.filter(c=>c.name==='consume_ai_action').length,0);const age=w.document.getElementById('kids-age');age.value='3-5';age.dispatchEvent(new w.Event('change'));release(await asyncSession());await tick();assert.equal(calls.filter(c=>c.name==='consume_ai_action').length,0);assert.equal(w.localStorage.getItem('match_dailyCount'),null);assert(!w.document.getElementById('kids-watch-dialog').open);d.window.close();
});
test('quota denial and connection errors leave the Kids result closed without a guest fallback',async()=>{
 for(const failed of [false,true]){const {w,d,calls}=await boot(failed?async()=>({error:{message:'offline'}}):undefined,{allowed:false});w.document.querySelector('#kids-grid [data-title-watch=bluey]').click();await tick();assert(!w.document.getElementById('kids-watch-dialog').open);assert(w.document.getElementById('kids-match-status').textContent);assert.equal(w.localStorage.getItem('match_dailyCount'),null);assert.equal(calls.filter(c=>c.name==='consume_ai_action').length,failed?0:1);d.window.close();}
});
