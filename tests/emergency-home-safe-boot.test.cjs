...
test('homepage emergency recovery is static and cannot start a JS freeze loop',()=>{
  ...
  assert.doesNotMatch(html,/<script\b[^>]*src=["'][^"']*googletagmanager/i);
  assert.doesNotMatch(html,/<script\b[^>]*src=["'][^"']*adsbygoogle/i);
  assert.doesNotMatch(html,/<script\b[^>]*src=["'][^"']*supabase/i);
