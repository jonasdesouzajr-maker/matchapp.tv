...
test('Home startup avoids delayed boot locks, stale cache keys and duplicate header owners',()=>{
 ...
 const versions={'build-meta.js':'20260920-design2','matchapp-ia.js':'20260920-freeze-final1','settings.js':'20260920-freeze8','app.js':'20260920-design2','catalog-media.js':'20260920-freeze-final1','lazy.js':'20260920-design2'};
...
test('Home noncritical enrichment is staggered instead of timing out together',()=>{
 ...
 assert.match(html,/\/poster-wall\.css\?v=20260920-design2/);
...
test('Home header is visible without JavaScript and avoids filtered 8K SVGs',()=>{
 ...
 assert.match(html,/\/matchapp-ia\.css\?v=20260920-design2/);
 assert.match(html,/class="ma-brand-orb" src="\/assets\/brand\/matchapp-official-icon-512\.webp\?v=20260920-circle2"/);
...
test('Home editorial scripts do not preload hundreds of pixels before view',()=>{
 ...
 assert.match(meta,/final-wiring\.js\?v=20260920-design2/);
});
...
test('Home removes the nonfunctional trending fold bar and keeps autoplay bounded',()=>{
 ...
 assert.match(app,/autoDelay = vp\.id === 'marquee-viewport' \? 3200 : 6500/);
 assert.match(news,/AUTO_FIRST_MS=2000/);
 assert.match(news,/AUTO_MS=3200/);
});