...
test('startup visuals do not run full-screen animation-frame loops',()=>{
  assert.doesNotMatch(ambient,/requestAnimationFrame/,'ambient background must not schedule a continuous frame loop');
  ...
  assert.doesNotMatch(marquee,/requestAnimationFrame/,'Top Titles fallback must not run a 60 FPS frame loop');
