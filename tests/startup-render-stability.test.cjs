...
test('Home startup avoids delayed boot locks, stale cache keys and duplicate header owners',()=>{
 ...
 assert.ok(html.includes('/build-meta.js?v=20260926-playpending1'),'stale shared runtime build-meta.js');
 assert.ok(html.includes('/settings.js?v=20260925-guarantee3'),'stale shared runtime settings.js');
 assert.ok(html.includes('/catalog-media.js?v=20260926-catalogscale1'),'stale shared runtime catalog-media.js');
 assert.ok(html.includes('/app.js?v=20260926-catalogscale1'),'stale shared runtime app.js');
...
test('Home noncritical enrichment is staggered instead of timing out together',()=>{
 ...
 assert.match(html,/\/poster-wall\.js\?v=\d{8}-[\w-]+/);
 assert.match(html,/\/title-captions\.js\?v=20260921-ui2/);
 assert.match(html,/\/catalog-media\.js\?v=\d{8}-[\w-]+/);
 assert.match(html,/\/poster-wall\.css\?v=\d{8}-[\w-]+/);
