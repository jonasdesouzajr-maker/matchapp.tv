...
test('core CTA, quota loading, trending and separate AI experiences remain intact',()=>{
  ...
  assert.match(s,/Checking daily allowance/i);
...
test('both Android Studio apps carry stabilized release marker without changing route separation',()=>{
  ...
  assert.match(mainGradle,/versionCode = 16/);
  assert.match(mainGradle,/versionName = "1\.1\.8"/);
  assert.match(kidsGradle,/versionCode = 16/);
  assert.match(kidsGradle,/versionName = "1\.1\.8"/);
  assert.match(main,/https:\/\/matchapp\.tv\/\?utm_source=android_app&appBuild=16/);
  assert.match(kids,/https:\/\/matchapp\.tv\/kids\/\?utm_source=android_kids_app&appBuild=16/);
