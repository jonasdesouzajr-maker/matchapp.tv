...
test('all original trending covers remain keyboard reachable after visible captions are removed',async()=>{
 ...
 try{w.MATCH_LANG='en';w.eval(fs.readFileSync(path.join(__dirname,'../title-captions.js'),'utf8'));w.document.dispatchEvent(new w.Event('DOMContentLoaded'));await Promise.resolve();const tiles=w.document.querySelectorAll('.marquee-item');assert.equal(tiles[0].getAttribute('role'),'button');assert.equal(tiles[0].tabIndex,0);assert.equal(tiles[0].getAttribute('aria-label'),'A Film');assert.equal(tiles[1].tabIndex,-1);assert.equal(w.document.querySelector('.marquee-title'),null);}finally{w.close();}
});
