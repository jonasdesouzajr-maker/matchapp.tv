...
14. **Cross-platform matching parity is mandatory.** Every Matching System and Ask Ai fix must be verified as one shared behavior across desktop browsers, macOS/Safari, iPhone/iOS browsers/PWA, Android browsers/PWA, Kids Mode, and both Android Studio app variants where applicable. Do not ship a phone-only workaround when the same matching path is used elsewhere. Keep source verification, safety, no-repeat rules, loader behavior and cache-version rollout consistent across those clients, without loosening Kids safety.
